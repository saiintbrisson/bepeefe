...
use object::{ObjectSection, Section, SectionIndex};
...
pub fn load_btf(section: &Section<'_, '_>) -> Result<Btf, ()> {
...
pub fn load_maps(btf: &Btf, maps_sec: SectionIndex) -> Vec<BpfMapDeclaration<'_>> {
    let Some((name_off, _)) = btf
        .strings
        .iter()
    ...
    let btf_ty = btf
        .types
        .values()
        .find(|ty| ty.name_off == *name_off)
        .expect("maps section does not have corresponding BTF type");
    ...
    for info in secinfos {
        ...
        let map_name = btf.strings.get(&btf_ty.name_off);

        let btf_ty = btf.get_type(*ty).expect("missing map definition");
        let BtfKind::Struct { members, .. } = &btf_ty.kind else {
            continue;
        };

        let Some(map_name) = map_name.or_else(|| btf.strings.get(&btf_ty.name_off)) else {
            panic!("map missing name");
        };

        let mut map_builder = BpfMapDeclaration {
            name: map_name,
            r#type: None,
            sec_idx: maps_sec.0,
        ...
        for member in members {
            let Some(name) = btf.strings.get(&member.name_off) else {
...
#[derive(Debug)]
pub struct BpfMapDeclaration<'b> {
    pub name: &'b CStr,
