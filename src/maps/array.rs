#![allow(
    ...
    reason = "Mutex::lock only fails if poisoned, region indexing is guarded by check_bounds"
)]
...
pub struct Array {
    ...
    /// The resulting value layout, without padding. Its size is provided by the
    /// map configuration.
    element_layout: Layout,
    /// The padded array element layout. Its size is padded to align,
    /// representing the element stride.
...
impl Array {
    pub fn new(max_entries: u32, value_size: u32) -> Result<Self, &'static str> {
        ...
        let element_layout =
            Layout::from_size_align(value_size, 8).map_err(|_| "invalid value layout")?;
        let stride_layout = element_layout.pad_to_align();

        let total = stride_layout
            .size()
    ...
    pub fn update(&self, key: &[u8], value: &[u8]) -> Result<(), RuntimeError> {
        if value.len() != self.element_layout.size() {
            return Err(RuntimeError::MapWrongValueSize {
                expected: self.element_layout.size(),
                got: value.len(),
            });
        }

        let offset = self.lookup(key).ok_or(RuntimeError::MapKeyNotFound)?;
    ...
    pub fn for_each_entry(&self, mut f: impl FnMut(&[u8], &[u8])) {
        ...
        for i in 0..self.max_entries {
            ...
            let offset = i * stride;
            f(&key, &data[offset..offset + value_size]);
        }
    }
