...
fn decode(world: &'_ World, space: TaggetPtrType, src: u64) -> Option<(usize, &'_ [u8])> {
    let tag = TaggedPtr::try_decode(src)?;
    match (space, tag) {
        (TaggetPtrType::Kernel, TaggedPtr::Kernel { offset }) => {
            Some((offset as usize, world.kernel.as_ref()))
        }
        (TaggetPtrType::User, TaggedPtr::User { offset }) => {
            Some((offset as usize, world.user.as_ref()))
        }
        _ => None,
    }
}

fn read_at(world: &World, space: TaggetPtrType, src: u64, len: usize) -> Option<Vec<u8>> {
    let (offset, slice) = decode(world, space, src)?;
    let end = offset.checked_add(len)?;
    slice.get(offset..end).map(<[u8]>::to_vec)
}

fn read_str(world: &World, space: TaggetPtrType, src: u64, cap: usize) -> Option<Vec<u8>> {
    let (offset, slice) = decode(world, space, src)?;
...
pub(crate) fn probe_read(
    ...
    let bytes = read_at(&state.world(), space, src, len);
    match bytes {
        Some(bytes) => {
...
pub(crate) fn probe_read_str(
    ...
    let bytes = read_str(&state.world(), space, src, cap);
    match bytes {
        Some(bytes) => {
...
mod tests {
    ...
    fn world_kernel(bytes: &[u8]) -> World {
        World::default().with_kernel(Arc::<[u8]>::from(bytes))
    }

    fn world_user(bytes: &[u8]) -> World {
        World::default().with_user(Arc::<[u8]>::from(bytes))
    }

    #[test]
    fn read_at_returns_bytes() {
        let world = world_kernel(b"hello world");
        let src = TaggedPtr::kernel(6);
        assert_eq!(
            read_at(&world, TaggetPtrType::Kernel, src, 5).as_deref(),
            Some(&b"world"[..])
        );
    }
    ...
    #[test]
    fn read_at_oob() {
        let world = world_kernel(b"hi");
        let src = TaggedPtr::kernel(0);
        assert!(read_at(&world, TaggetPtrType::Kernel, src, 10).is_none());
    }

    #[test]
    fn read_at_wrong_space() {
        let world = world_kernel(b"hello");
        let src = TaggedPtr::user(0);
        assert!(read_at(&world, TaggetPtrType::Kernel, src, 5).is_none());
    }

    #[test]
    fn read_at_null() {
        let world = world_kernel(b"hello");
        assert!(read_at(&world, TaggetPtrType::Kernel, 0, 1).is_none());
    }

    #[test]
    fn read_at_user_space() {
        let world = world_user(b"abc");
        let src = TaggedPtr::user(0);
        assert_eq!(
            read_at(&world, TaggetPtrType::User, src, 3).as_deref(),
            Some(&b"abc"[..])
        );
    }
    ...
    fn read_str_finds_nul() {
        let world = world_kernel(b"hi\0extra");
        let src = TaggedPtr::kernel(0);
        assert_eq!(
            read_str(&world, TaggetPtrType::Kernel, src, 16).as_deref(),
            Some(&b"hi\0"[..])
        );
    }
    ...
    fn read_str_truncates_at_cap() {
        ...
        let src = TaggedPtr::kernel(0);
        assert_eq!(
            read_str(&world, TaggetPtrType::Kernel, src, 4).as_deref(),
            Some(&b"hel\0"[..])
        );
    }
    ...
    fn read_str_no_nul_within_region() {
        let world = world_kernel(b"abc");
        let src = TaggedPtr::kernel(0);
        // cap=8 means up to 7 bytes + NUL; region only has 3, so we get those plus NUL.
        assert_eq!(
            read_str(&world, TaggetPtrType::Kernel, src, 8).as_deref(),
            Some(&b"abc\0"[..])
        );
    }
}
